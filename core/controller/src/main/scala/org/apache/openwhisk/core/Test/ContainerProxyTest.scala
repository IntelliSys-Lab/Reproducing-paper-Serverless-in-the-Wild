...
import org.apache.openwhisk.core.containerpool.WindowMap._
...
class ContainerProxyTests
  ...
  def preWarm(machine: ActorRef) = {
    ...
    println("Has transmitted")
    expectPreWarmed(exec.kind)
    //expectMsg(Transition(machine, Starting, Started))
    println("Has transmitted, too："+expectMsg(Transition(machine, Starting, Started)))
  }

  /** Run the common action on the state-machine, assumes good cases */
  def run(machine: ActorRef, currentState: ContainerState) = {
    machine ! Run(action, message)
    println("start: "+ expectMsg(Transition(machine, currentState, Running)))
    println("then:"+expectWarmed(invocationNamespace.name, action))
    println(expectMsg(Transition(machine, Running, Ready)))
  }
  ...
  it should "run a container which has been started before, write an active ack, write to the store, pause and remove the container" in within(
    ...
    preWarm(machine)
    run(machine, Started)   //搞清楚run函数，还有各种State
 ...
 */
  ...
  it should "resend the job to the parent if /run fails connection after Ready -> Running" in within(timeout) {
    ...
    println("parameter:"+WindowMap.getWindow(Run(action, msg)))
    println(MAP)
    println("parameter:"+WindowMap.getWindow(Run(action, message))) //查一下Run函数怎么写的
    println(MAP)
