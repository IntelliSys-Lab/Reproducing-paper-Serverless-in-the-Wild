...
object histogram{
  ...
  var Window = scala.collection.mutable.Map("ActionrName" -> (0,10)) //保存每个action的两个window
  ...
  //Histogram Policy
  def HistogramPolicy(actionMetaData: ExecutableWhiskActionMetaData): (Int, Int) = {
    ...
    if(idletime<Bounds){
      ...
      if (idleTimeTotalNumber(actionType) <= 3) { //次数<=10时，使用第二种policy(a standard keep-alive approach):
      ...
      if (idleTimeTotalNumber(actionType) > 3) { //次数>10时，使用第一种policy(Range-limited histogram):
      ...
      Window(actionMetaData.fullyQualifiedName(false).toString) = (preWarmWindow, keepAliveWindow)
...
class ShardingContainerPoolBalancer(
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
    ...
    //记录并计算action的window
    histogram.HistogramPolicy(action)
    println("Finish hist")
    ...
    logging.info(
      this,
      s"ActionName: '${action.name}, PreWarmWindow parameter:  ${msg.preWarmParameter}, KeepAliveWindow:  '${msg.keepAliveParameter} ")
