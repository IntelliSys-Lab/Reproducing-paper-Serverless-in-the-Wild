...
case object WindowMap {
  ...
  def getWindow(job:Run) = {
    var prewarm = job.msg.preWarmParameter
    var keepalive = job.msg.keepAliveParameter
    MAP(job.action.fullyQualifiedName(false).toString)=(prewarm,keepalive)
    (prewarm,keepalive)
  }
}
...
class ContainerPool(childFactory: ActorRefFactory => ActorRef,
  ...
  def receive: Receive = {
    ...
    case NeedWork(data: PreWarmedData) =>
      ...
      logging.info(
        this,
        s" Pool has received a Need Work Message. PreWarmPool:  ${prewarmedPool}. ")


    // 在XX分钟后，prewarm一个容器（需要prewarm值、action名称，即data信息）
    case PreWarmMessage(whiskAction: ExecutableWhiskAction) =>
      logging.info(
        this,
        s" Pool has received a PreWarmMessage. actionName: ${whiskAction.name}. ")
      val actionName = whiskAction.fullyQualifiedName(false)

      var preWarmWindow = WindowMap.MAP.get(actionName.toString).get._1

      //然后：创建一个新prewarm容器
      val kind = whiskAction.exec.kind
      val memory = whiskAction.limits.memory.megabytes.MB
      val ttl = None
      println("ttl: "+ttl)
      ...
      logging.info(
        this,
        s"ActionName: '${whiskAction.name}, This Container Will be PreWarmed after:  ${preWarmWindow}  minutes. ")
      ...
      val process = new Runnable {
        override def run() = {
          logging.info(
            this,
            s"ActionName: '${whiskAction.name}, Judging Container .... ")

          if(NumofPre.PreMAP.get(actionName.toString).get._1 == 0) { //只有当该action对应的容器数量为0时，才会创建新容器
            NumofPre.PreMAP(actionName.toString) = (1, 1) //预热容器数量 + 1 ，且将状态码调整为1
            logging.info(
              this,
              s"ActionName: '${whiskAction.name}, This Container is being PreWarmed. ")
            prewarmContainer(whiskAction.exec, memory, ttl) //这里要加上containerID！！！==> 没办法加，只能在Starting状态下加上。
          }
            //想个办法，再搞一个全局变量，因为对于我们要控制的容器，一定是要先在这里 being prewarm，然后再进入paused状态，所以可以在这里添加一个信号，being pre后，信号从0变为1。
            // 在paused状态，先读取信号，如果信号为1，就知道需要将map值 -1。如果为0，就证明是其他预热容器，就不操作。

          else{
            logging.info(
              this,
              s"ActionName: '${whiskAction.name}, This Container Will Not be PreWarmed. ")
          }

          println("PreMap in Pool:  "+ NumofPre.PreMAP)
        }
      }

      delayRun.schedule(process, preWarmWindow, TimeUnit.MINUTES) // 第二个参数为延时时间
        //只有当预热容器数量为0时，才会新建pre-warm容器
