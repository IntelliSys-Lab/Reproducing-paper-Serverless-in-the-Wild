...
case object Temp1 extends ContainerState
...
class ContainerProxy(factory: (TransactionId,
  ...
  when(Uninitialized) {
    ...
    case Event(job: Run, _) =>
      ...
      if (NumofPre.PreMAP.get(job.action.fullyQualifiedName(false).toString) == None) {
        println("The PreMap is Initializing")
        NumofPre.PreMAP(job.action.fullyQualifiedName(false).toString) = (1, 0)
        println(NumofPre.PreMAP)
      }
      else {
        var x = NumofPre.PreMAP.get(job.action.fullyQualifiedName(false).toString).get._1 + 1
        var y = NumofPre.PreMAP.get(job.action.fullyQualifiedName(false).toString).get._2
        NumofPre.PreMAP(job.action.fullyQualifiedName(false).toString) = (x, y)
        println("The PreMap has already been Initialized")
        println(NumofPre.PreMAP)
      }
  ...
  when(Starting) {
    ...
    case Event(completed: PreWarmCompleted, _) =>
      context.parent ! NeedWork(completed.data)   //给pool发消息，说明prewarm完成，prewarm pool +1，prewarm starting pool -1.
      logging.info(
        this,
        s" This Container has been PreWarmed. ContainerID: ${completed.data.container.containerId} .Data Kind:${completed.data.kind}")
  ...
  when(Started,stateTimeout = unusedTimeout) {       //prewarm容器一直在这个状态待机，直到收到job: Run。【这样是违反keepalive机制的，应该替换。因此，让starting直接到paused】
    case Event(job: Run, data: PreWarmedData) =>
      implicit val transid = job.msg.transid
      activeCount += 1
      logging.info(
        this,
        s" This PreWarmed Container has received a running job. ContainerID: ${data.container.containerId} action name: ${job.action.name}.")
    ...
    case Event(StateTimeout, data: PreWarmedData) =>
      ...
      logging.info(
        this,
        s" This PreWarmed Container is too old to keep alive. ContainerID: ${data.container.containerId}. It has been kept for ${unusedTimeout} minutes")
    ...
    case Event(RunCompleted, data: WarmedData) =>
      ...
      if (requestWork(data) || activeCount > 0) {
        ...
        if(WindowMap.MAP.get(data.action.fullyQualifiedName(false).toString).get._1!=0){ //要是pre-warm为0，就不销毁容器（也就不发送预热信号了）
          context.parent ! PreWarmMessage(data.action)
        }
  ...
  when(Ready, stateTimeout = pauseGrace) {  //pauseGrace：用于很短暂的暂停，以防在很短时间内，又来了新的job: Run
    ...
    case Event(StateTimeout, data: WarmedData) =>
      ...
      if(WindowMap.MAP.get(data.action.fullyQualifiedName(false).toString).get._1!=0){
        //让容器数量 - 1 。因为该容器即将destroy
        var x = NumofPre.PreMAP.get(data.action.fullyQualifiedName(false).toString).get._1 - 1
        var y = NumofPre.PreMAP.get(data.action.fullyQualifiedName(false).toString).get._2
        if(x>=0) {
          NumofPre.PreMAP(data.action.fullyQualifiedName(false).toString) = (x, y)
        }
        else{
          NumofPre.PreMAP(data.action.fullyQualifiedName(false).toString) = (0, y)
        }
        println("PreMap in State Ready:  "+ NumofPre.PreMAP)

        logging.info(
          this,
          s" This Container Will be destroyed. It will be prewarmed after a few minutes. ActionName: ${data.action.name}, ContainerID: ${data.container.containerId}")
        destroyContainer(data, false) //执行完任务后，直接摧毁容器。状态转移到removing state
      }
      else{
        data.container.suspend()(TransactionId.invokerNanny).map(_ => ContainerPaused).pipeTo(self)
        goto(Pausing)
  ...
  when(Pausing) {
    //原来的State：
    case Event(ContainerPaused, data: WarmedData)   => goto(Paused)  //不能直接去paused，进行keep-alive，应该进入新的阶段。
    case Event(_: FailureMessage, data: WarmedData) => destroyContainer(data, false)
    case _                                          => delay
  }

  when(Paused, stateTimeout = unusedTimeout) {   //keep alive机制在这里
    ...
    case Event(job: Run, data: PreWarmedData) =>   //针对Prewarm的容器，把Started转到Running的条件搬过来，应该是通用的。
      implicit val transid = job.msg.transid
      logging.info(
        this,
    ...
    case Event(StateTimeout | Remove, data: WarmedData) =>
      ...
      println("PreMap in State Paused (WarmedData):  "+ NumofPre.PreMAP)

      logging.info(
        this,
        s" This Paused Container is too old to keep alive. ActionName: ${data.action.name}, ContainerID: ${data.container.containerId}. It has been kept for ${unusedTimeout} minutes")
      destroyContainer(data, false)
    ...
    case Event(StateTimeout, data: PreWarmedData) =>
      ...
      NumofPre.actionName = "xx"
      NumofPre.PreMAP("xx") = (10,0)
      for(kv<-NumofPre.PreMAP)
        if(kv._2._2 ==1){
          NumofPre.actionName = kv._1
          //pre-warm容器到达寿命，容器数量 -1
          var x = NumofPre.PreMAP.get(NumofPre.actionName).get._1 - 1
          NumofPre.PreMAP(NumofPre.actionName) = (x, 0)
        }

      println("PreMap in State Paused (PreWarmedData):  "+ NumofPre.PreMAP)
