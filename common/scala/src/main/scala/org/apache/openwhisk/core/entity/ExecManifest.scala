...
object ExecManifest {
   ...
   */
  def requiredProperties = Map(WhiskConfig.runtimesManifest -> null)
   ...
   */
  def initialize(config: WhiskConfig, manifestOverride: Option[String] = None): Try[Runtimes] = {
   ...
   */
   def runtimes(config: JsObject, runtimeManifestConfig: RuntimeManifestConfig): Try[Runtimes] = Try {
   ...
   */
   case class RuntimeManifestConfig(bypassPullForLocalImages: Option[Boolean] = None,
   ...
   */
  case class StemCell(initialCount: Int,
                                        memory: ByteSize,
   ...
   */
  case class RuntimeFamily(name: String, versions: Set[RuntimeManifest])
   ...
   */
  case class Runtimes(runtimes: Set[RuntimeFamily],
  ...
  implicit val imageNameSerdes: RootJsonFormat[ImageName] = jsonFormat4(ImageName.apply)

  implicit val ttlSerdes: RootJsonFormat[FiniteDuration] = new RootJsonFormat[FiniteDuration] {
  ...
  implicit val reactivePrewarmingConfigSerdes: RootJsonFormat[ReactivePrewarmingConfig] = jsonFormat5(
    ReactivePrewarmingConfig.apply)

  implicit val stemCellSerdes = new RootJsonFormat[StemCell] {
  ...
  implicit val runtimeManifestSerdes: RootJsonFormat[RuntimeManifest] = jsonFormat8(RuntimeManifest)
