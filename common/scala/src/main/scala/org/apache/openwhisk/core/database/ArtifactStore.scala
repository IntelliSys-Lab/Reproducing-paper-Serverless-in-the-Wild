...
trait ArtifactStore[DocumentAbstraction] {
   ...
   */
  def put(d: DocumentAbstraction)(implicit transid: TransactionId): Future[DocInfo]
   ...
   */
  def del(doc: DocInfo)(implicit transid: TransactionId): Future[Boolean]
   ...
   */
  def putAndAttach[A <: DocumentAbstraction](
    d: A,
    update: (A, Attached) => A,
