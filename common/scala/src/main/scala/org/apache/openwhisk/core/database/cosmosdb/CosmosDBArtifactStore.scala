...
class CosmosDBArtifactStore[DocumentAbstraction <: DocumentSerializer](protected val collName: String,
  ...
  override  def put(d: DocumentAbstraction)(implicit transid: TransactionId): Future[DocInfo] = {
  ...
  override  def del(doc: DocInfo)(implicit transid: TransactionId): Future[Boolean] = {
  ...
  override def putAndAttach[A <: DocumentAbstraction](
    doc: A,
    update: (A, Attached) => A,
